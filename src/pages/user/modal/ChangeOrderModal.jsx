...
const ChangeOrderModal = ({ isOpen, onClose, orderId, onOrderChanged }) => {
	...
	useEffect(() => {
		const fetchRefundTypes = async () => {
			try {
				const response = await axios.get(`${import.meta.env.VITE_BUYER_REST_API_URL}/my/user/order/refundTypes`);
		...
		const fetchOrderOptions = async () => {
			try {
				const response = await axios.get(`${import.meta.env.VITE_BUYER_REST_API_URL}/my/user/order/pay?orderId=${orderId}`);
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const response = await axios.post(
				`${import.meta.env.VITE_BUYER_REST_API_URL}/my/user/order/refund`, 
				formData,
				{
					headers: {
