...
const ReviewEditModal = ({ reviewData, onClose, onUpdate }) => {
  ...
  useEffect(() => {
    if (reviewData) {
      ...
      if (reviewData.imageUrls && reviewData.imageUrls.length > 0) {
        setImagePreview(reviewData.imageUrls);
      } else {
        setImagePreview([]);
      }
  ...
  const handleRemoveImage = (index) => {
    ...
    // 새로 업로드한 이미지인 경우에만 URL 해제
    if (index >= reviewData.imageUrls?.length) {
    ...
    if (selectedImages.length > 0) {
      ...
      const adjustedIndex = index - (reviewData.imageUrls?.length || 0);
      if (adjustedIndex >= 0) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 삭제할 이미지 URL 목록
      const removedImageUrls = (reviewData.imageUrls || []).filter(
        url => !imagePreview.includes(url)
      );
  ...
  const productImage = reviewData?.productImage 
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white w-full max-w-md rounded-lg overflow-hidden">
        ...
        <div className="flex justify-between items-center px-4 py-3 border-b">
          <h2 className="text-lg font-medium">리뷰 수정</h2>
        ...
        <div className="p-4 border-b">
          <div className="flex">
            <div className="w-20 h-20 bg-gray-100 rounded overflow-hidden mr-3">
              <img 
                src={productImage} 
                alt={reviewData?.productName || '상품 이미지'} 
                className="w-full h-full object-cover" 
              />
            </div>
            <div>
              <p className="font-medium">{reviewData?.productName || '상품명'}</p>
              <p className="text-sm text-gray-500">
                {reviewData?.orderOptions && reviewData.orderOptions.map((option, idx) => (
                  <span key={idx}>
                    {option.optionName} {option.quantity}개
                    {idx < reviewData.orderOptions.length - 1 ? ', ' : ''}
                  </span>
                ))}
              </p>
            </div>
