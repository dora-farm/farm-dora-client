...
export function useWishlist(userId, previewMode = false) {
  ...
  const loadWishlistItems = async () => {
    setIsLoading(true);
    try {
      const endpoint = previewMode ? `http://localhost:8080/api/my/user/dashboard/preview` : `http://localhost:8080/api/my/user/wishlist/items`;
      const response = await axios.get(endpoint, { params: { userId } });

      const limitedData = previewMode ? response.data.slice(0, 4) : response.data.data;
